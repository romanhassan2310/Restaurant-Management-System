...
export default function QrOrderPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const response = await api.post(`/qr/${token}/orders`, { customerName, customerPhone, items: cart.map((item) => ({ product: item.product._id, variant: item.variant?._id, quantity: item.quantity, modifiers: item.modifiers.map((modifier) => modifier._id) })) });
      setOrderId(response.data.data.order._id); setStatus(response.data.data.order.status); setCart([]); setMessage('Order sent to the restaurant.');
